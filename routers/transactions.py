...
# READ (user's own transactions with filtering, sorting, and pagination)
@router.get("/")
    ...
    pipeline = [
        ...
        # Shape the final output
        {
            "$project": {
                ...
                "created_at": 1,
                "category_details": {
                    "id": {"$ifNull": [{"$toString": "$category_id"}, ""]},
                    "name": {"$ifNull": ["$category.category_name", "Others"]},
                    "icon": {"$ifNull": ["$category.icon", ""]},
                    "type": {"$ifNull": ["$category.type", "$type"]},
                    "group_id": {"$ifNull": ["$category.group_id", ""]},
                    "group_name": {"$ifNull": ["$group.group_name", "Others"]}
                }
            }
        },
