...
async def get_conversation_history(user_id: str, limit: int = 20):
    """
    Retrieve the last N messages from the conversation history.
    Returns messages in chronological order (oldest first).
    """
    try:
        # Get messages sorted by timestamp (newest first), then reverse
...
async def generate_text(
    ...
    try:
        ...
        # Retrieve conversation history (including the message we just saved)
        history = await get_conversation_history(user_id, limit=20)
        
        # Build the conversation history for Gemini
        # Exclude the last message (current prompt) since we'll send it separately
        chat_history = []
        for msg in history[:-1]:  # Exclude the last message (current user prompt)
            chat_history.append({
                "role": msg["role"],
                "parts": [msg["content"]]
            })
