...
# get the top categories based on transactions
@router.get("/most-used")
async def get_top_categories(current_user: dict = Depends(get_current_user)):
    user_id = current_user["_id"]
    ...
    pipeline = [
        ...
        {"$match": {"category_object_id": {"$ne": None}}},

        # Count how many times each category is used
        {"$group": {
            "_id": "$category_object_id",  # Group by this field
            "usageCount": {"$sum": 1} # count items in each group
        }},

        # Join with categories collection
        {"$lookup": {
            "from": "categories",
            "localField": "_id",           # ← Fixed: use _id from $group
            "foreignField": "_id",
            "as": "category"
        }},

        # Unpack the category array
        {"$unwind": "$category"},
    ...
    for item in result:
        ...
        item["category"]["_id"] = str(item["category"]["_id"])
...
async def update_category(
    ...
    updated_category = category_body.dict(
